...
**`time_in_hr_zone` is not the same number on both messages.** `lap` is 57 and
`session` is 65 — and **session field 57 is `avg_temperature`**, a `sint8` in
degrees Celsius. Using the lap number on the session declares a temperature
field as a six-element `uint32` array. Spin measures no temperature, so nothing
would have looked wrong locally; the damage is entirely in the reader.

`Spin/Tests/ActivityWriter_test.cpp` asserts the session did *not* write
anything into field 57, which is the check that catches this class of mistake.
