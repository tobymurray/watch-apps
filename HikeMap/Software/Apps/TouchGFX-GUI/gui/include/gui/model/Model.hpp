...
// The shared map layer (watch-apps/MapKit). Everything map-related this app
// does lives behind MapSession; the only thing that is this app's business is
// feeding it GPS samples and telling the map face when to redraw.
...
public:
    ...
    // -----------------------------------------------------------------
    // Live map. All of it lives here, in the Model, because TouchGFX
    // destroys and re-creates screens on every transition -- a map that
    // belonged to the Track screen would lose its open pack handle and its
    // whole breadcrumb every time the wearer scrolled to another face.
    //
    // The tile cache is separate and file-static in Model.cpp, deliberately
    // outside the FrontendHeap object, so the linker arbitrates the GUI RAM
