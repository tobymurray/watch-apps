...
namespace
{
// The map tile cache: MapKit::TileCache::SLOTS x 64 KiB of pixels.
//
// File-static on purpose, and NOT a member of Model (which lives inside the
// TouchGFX FrontendHeap object). That puts it in .bss, where the linker
// arbitrates the GUI RAM budget at build time -- and it really does: four
// slots overflowed the device RAM region by 160 KB on top of this GUI's
// existing statics, which is how SLOTS came to be 1. A cache that does not
// fit should fail the build, not the watch.
MapKit::TileCache sTileCache;
}
...
Model::Model()
    ...
    , mSrvSender(mKernel)
    , mMap(mKernel, sTileCache, "HikeMap")
{
...
bool Model::customMessageHandler(SDK::MessageBase* message)
{
    switch (message->getType()) {
        ...
        case CustomMessage::GPS_POSITION: {
            // Everything map-related this app does is these three lines. The
            // breadcrumb only grows while the activity is actually recording,
            // so a paused activity does not draw a straight line across the
            // gap where the wearer stood still.
            auto* msg = static_cast<CustomMessage::GpsPosition*>(message);
            mMap.onPosition(msg->latitude, msg->longitude, msg->fix,
        ...
        case CustomMessage::TRACK_STATE_UPDATE: {
            ...
            if (mTrackState != msg->state) {
                if (mTrackState == Track::State::INACTIVE) {
                    mMap.resetTrace();   // new activity, new breadcrumb
                }
