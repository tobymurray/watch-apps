...
private:
    ...
    /// The hardware engine. Used instead of mPwm when a marker file selects it:
    /// same waveform on the same pin, generated by a timer and a DMA channel
    /// with no CPU involvement at all.
    Pwm::DmaPwm mDma;
    /// How the thread passes time while the hardware engine runs, chosen from
    /// the rate probe rather than assumed. See DmaPwm::Idle.
    Pwm::Idle   mRunIdle = Pwm::Idle::Yield;
    bool        mUseDma = false;
    ...
    /// Hands a duty to whichever engine is running.
    void driveDuty(uint8_t duty);
