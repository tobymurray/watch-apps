...
| | |
|---|---|
| [`MapKit`](MapKit) | The map: tiles, the cache and the RAM arbitration, for `BikeMap`, `HikeMap` and `RunMap`, which are otherwise the same app three times over. |
| [`SettingsKit`](SettingsKit) | Everything that touches the watch's live settings struct and `settings.json`, shared by `NotifyToggle`, `UnitToggle` and `SettingsEditor`. GUI only: every file in it either carries a raw firmware address or is reached by something that does. |
...
| [`InscribedDisc`](InscribedDisc) | The GUI kit under those same Rust renderers: the round panel's geometry, the `ABGR2222` surface that clips every draw to the glass rather than to the square framebuffer, two widgets, and a PNG preview that shows exactly the 64 colours the panel can. Distilled from the seven `CustomGUI` apps, each of which had written its own copy — the framebuffer's `DrawTarget` impl was byte-identical in all seven. Shaped so the Rust half could be published for round low-bit-depth panels generally; the C ABI, `GuiShell.hpp` and `inscribed-disc.cmake` are this repository's glue and stay here. [`InscribedDisc/Docs/DESIGN.md`](InscribedDisc/Docs/DESIGN.md) is the design record. |
