 ...
 * constructor arguments. Chrono targets 1.3, where `MessageGuard` and the
 * nullary `make_msg` exist but that last step does not, so it carries its own.
 *
 * `Chrono::sendMsg<T>(kernel)` is the whole of what this app needs from that
 ...
 *
 * If Chrono is ever moved to an SDK of 1.4 or later, this header can be deleted
