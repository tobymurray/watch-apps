...
fn main() {
    ...
    let mut window = Window::new("UnitToggle sim (240x240, ABGR2222, round)", &output);

    println!("Interactive sim. SPACE/R1 = switch units, U = unreadable, N = not saved, L = live only, S = unreadable settings, F = unsupported firmware, ESC/R2 = quit.");

    let mut buf = vec![0u8; (W * H) as usize];
    let mut st = State { imperial: 0, known: 1, status: 0, _pad: [0; 1] };

    unit_toggle_gui::render(&mut buf, W, H, &st);
            ...
            match event {
                ...
                SimulatorEvent::KeyDown { keycode, .. } => match keycode {
                    Keycode::Space | Keycode::Return => {
                        st.known = 1;
                        st.status = 0;
                        st.imperial = if st.imperial == 0 { 1 } else { 0 };
                        unit_toggle_gui::render(&mut buf, W, H, &st);
                        blit(&mut display, &buf);
                    }
