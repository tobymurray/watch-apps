...
- **The nightstand night is now a SleepLab night.** It was the last thing only the
  probe could do: a session only opens on a *worn* epoch, so a watch correctly
  reported as off the wrist produced no night and no counts. `Nights/watching.csv`
  records those minutes whether or not a session opens, which closes gap G2 and
  retires probe night 3.
