...
| S13 | A Tier 0 probe night can set the movement thresholds, as four constants' TODOs claim. | **REFUTED** | 2026-08-18, by reading `ProbeLog.hpp` against those TODOs. The probe records `acc_n`, `acc_ts_span_ms`, `acc_max_gap_ms` and `acc_batches` — delivery statistics — and **no activity counts at all**. `WornGate::kMicroMovementFloor` says "this is the constant the *table night* exists to set … put the floor between the two distributions"; there are no count distributions in a probe night to put a floor between. Same for `NightAnalyser::kMovementFloor`, `SegmenterConfig::stillnessCountMax` and `activityCountMin`. **Four constants were pointed at a night that cannot set them, and it would have taken two nights to find out.** The worn and table nights have to be recorded with *SleepLab*, whose epoch row already carries `count` and `peak`. See `POST-MORTEM.md` § "Should the probe and SleepLab stay separate?" for the merge recommendation and its arithmetic. | 2026-08-18 |
...
- **The two probe nights are the wrong two nights.** S13 refutes the plan: the
  probe records no activity counts, so it cannot set the four movement constants
  whose TODOs point at it. Record the worn night and the table night with
  *SleepLab*. `POST-MORTEM.md` has the merge recommendation, the arithmetic, and
  the shortest sequence of nights that settles the most.
- **Every remaining row in §2 is UNVERIFIED**, and the ones that are not about
  counts can still share a night. See `POST-MORTEM.md`.
- **G4 in `POST-MORTEM.md` is the highest-value unbuilt thing.** Every summary now
