...
use squashlab_gui::scenes::scenes;

const W: u32 = 240;
const H: u32 = 240;

fn main() {
    let dir = std::env::args().nth(1).unwrap_or_else(|| "/tmp/squashlab_gui_preview".to_string());
    ...
    for (name, frame) in scenes() {
        ...
        squashlab_gui::render(&mut buf, W, H, &frame);
    ...
    let (w, h) = sheet_from_frames(&sheet, &frames, W, H, 5, Options { scale: 1, bezel: Bezel::Transparent })
