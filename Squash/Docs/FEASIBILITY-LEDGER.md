...
| P7 | The kernel owns `Apps/app_list.json` and rewrites it; hand-editing it to register a build does nothing. | **CONFIRMED** | Edited the Squash entry to point at a locally built `0.6.99` with the current APP_ID, launched a 5-second activity, and read the file back: it had reverted to `A1E5C38B7D2F9046` / `0.2.1` / `Squash_0.2.1.uapp`, and the activity ran the old build — `activity_20260903T092731.fit` exists and no `Debug/squash.log` does, which only a build carrying `SquashLog` would write. | 2026-09-03 |
| P8 | Copying a `.uapp` into the app's folder installs it. | **UNVERIFIED for Squash** | `SleepLab/Docs/ROLLOUT.md` documents it for Sleep Probe — copy the file in, then "Eject, unplug, power-cycle" — and that may hold there. Here a copy plus a hand-edited registry did not install anything (P7), but the watch was never power-cycled between the copy and the next launch, so the documented procedure was not actually followed and this is untested rather than refuted. | 2026-09-03 |
| P9 | Something in the install path removes `.uapp` files it did not write. | **UNVERIFIED — what did it is not known** | After `0.6.0` was installed from a second development host, `Apps/Squash/` held exactly one `.uapp` and `0.2.1`, `0.3.0`, `0.4.0` and the hand-copied `0.6.99` were all gone, with `app_list.json` naming the new one and the current APP_ID. The deletion is a fact; its cause is not. It was first written up here as the phone's doing, which was wrong — no phone install took place — and the honest version is that the installing host's own procedure, or a manual tidy-up, is equally consistent with what was observed. Naming the mechanism needs whoever ran it to say what it was. | 2026-09-03 |
