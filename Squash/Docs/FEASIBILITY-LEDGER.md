...
| P8 | Copying a `.uapp` into the app's folder installs it. | **REFUTED for Squash** | `SleepLab/Docs/ROLLOUT.md` documents it for Sleep Probe — copy the file in, then "Eject, unplug, power-cycle" — and that may still hold there. It does not hold here: see P9, where a phone install deleted every copied file. Kept because it was believed, acted on, and cost a wasted install and a misleading first-check failure. | 2026-09-03 |
| P9 | The phone is the installer, and its install **wipes** hand-copied `.uapp` files. | **CONFIRMED** | After a phone install of `0.6.0`, `Apps/Squash/` held exactly one `.uapp` — `Squash_0.6.0.uapp` — and `Squash_0.2.1.uapp`, `Squash_0.3.0.uapp`, `Squash_0.4.0.uapp` and the hand-copied `Squash_0.6.99.uapp` were all gone, with `app_list.json` naming the new one and the current APP_ID. So copying a file in was never going to install anything here, whatever P8 says about SleepProbe, and the two files that sat unregistered since 2026-09-01 were never installable either. | 2026-09-03 |
