...
| --- | --- | --- | --- | --- |
| P1 | The four host suites build and pass against an SDK carrying both `SDK::AppConfig` and `ImuFusionSource`. | **VALIDATED** | `Tools/docker-build.sh tests`: 4/4, including `squash-filesink-tests`, which self-skipped before. The SDK is `8cdb7314` (the revision `app-build.yml` pins) with `5df2033e` cherry-picked. | 2026-09-03 |
...
| P3 | The `.uapp` packs with the heart-rate sidecar linked in. | **VALIDATED** | `Tools/docker-build.sh app`: 404 384 bytes, ID `9E526672EAFB61B6`, CRC `0x0FF911D8`. | 2026-09-03 |
| P4 | `EffortKit` is genuinely `no_std` and allocation-free. | **VALIDATED** | `cargo build --release --target thumbv8m.main-none-eabihf`. It is the check that catches anything testable creeping in behind `feature = "std"`. | 2026-09-03 |
