...
| P8 | Copying a `.uapp` into the app's folder is the install mechanism, but it needs a power-cycle to take. | **LIKELY** | `SleepLab/Docs/ROLLOUT.md` documents exactly that for Sleep Probe: copy the file in, then "Eject, unplug, power-cycle." Not yet confirmed for Squash — and there is evidence against it being sufficient here, which is P9. | 2026-09-03 |
| P9 | A `.uapp` whose APP_ID the kernel has never seen may need the phone rather than a file copy. | **UNVERIFIED** | `Squash_0.3.0.uapp` and `Squash_0.4.0.uapp` carry the current APP_ID, have been in the folder since 2026-09-01, and are registered nowhere; `grep` for `9E526672EAFB61B6` across every file in `Apps/` finds nothing. The APP_ID changed in `4f0993a`, so the phone sees a new app rather than an update to a known one. A power-cycle with `0.6.99` present is what would settle it: if it registers, P8 holds and these two were simply never rebooted into; if it does not, the phone is the only installer. | 2026-09-03 |
