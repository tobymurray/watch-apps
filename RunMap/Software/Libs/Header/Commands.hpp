...
namespace CustomMessage {
    ...
    /**
     * @brief Live GPS position, Service --> GUI, for the map face.
     *
     * The stock app forwards only the fix *state* (GpsFix); the coordinates
     * stay in the Service, which is all the stock faces need. The map needs
     * where, not just whether.
     *
     * Decimal degrees, float, straight from the GPS_LOCATION parser -- that is
     * the parser's own width. It resolves ~1e-5 deg, about a metre; the map
     * quantises to ~1.7 m world pixels at z16, so nothing survives the trip
     * that a double would have kept.
     */
...
public:
    ...
    /// Live position for the map face. Sent at the same 1 Hz cadence as the
    /// clock and battery updates, and from the same place -- well inside the
    /// GUI's custom-message queue, which holds ten and is drained once per
    /// frame at 10 Hz.
    bool position(float latitude, float longitude, bool fix)
    {
        bool status = false;
        auto *msg = mKernel.comm.allocateMessage<CustomMessage::GpsPosition>();
        if (msg) {
