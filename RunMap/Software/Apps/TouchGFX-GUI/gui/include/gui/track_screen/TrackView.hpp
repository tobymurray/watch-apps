...
protected:
    ...
    // The live-map face is a hand-written extra beside the Designer-generated
    // faces: a member of this subclass, not of TrackViewBase, so adding it
    // needs no TouchGFX Designer round-trip. There is no Designer in this
    // environment, so that property is load-bearing, not a convenience.
    MapKit::TrackFaceMap mFaceMap;
