 ...
 * athlete id here, but equally a transit pass, a membership number or an
 * account token -- has to arrive from outside. The one path that works today
 * is the USB mass-storage volume: `Apps/<Folder>/` is writable from any
 * desktop, and is the same directory this app's own relative paths resolve
 * into, so a file the host writes is a file the app can open.
 *
...
/// and the BLE file-transfer service see it.
